...
public class SuperRoom {
     ...
     */
    public void removeState(RoomState state) {
    ...
    public void collapse(Random rand) throws GenerationFailedException {
        if (entropy() == 0) {
            throw new GenerationFailedException("Zero entropy!");
        }
    ...
    public void collapse(RoomState state) {
        if (!possibleStates.contains(state)) {
            throw new IllegalArgumentException("State not possible!");
        }
        collapsed = true;
    ...
    public RoomState getState() {
        if (!collapsed) {
            throw new IllegalStateException("Tile not collapsed!");
        }
