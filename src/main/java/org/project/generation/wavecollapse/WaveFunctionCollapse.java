    ...
    public static SuperRoom getSuperRoomWithLowestEntropy(SuperRoom[][] superRooms, Random rand) {
        ...
        for (SuperRoom[] row : superRooms) {
            for (SuperRoom superRoom : row) {
                ...
                } else if (superRoom.entropy() == lowestEntropy) {
                    candidates.add(superRoom);
                }
            }
        }
    ...
    public static Room floodFill(SuperRoom[][] superRooms, int x, int y, int mapSize, int minRoomCount) {
        ...
        while (!superRoomQueue.isEmpty()) {
            ...
            for (int direction = 0; direction < 4; direction++) {
                ...
                if (otherX < 0 || otherX >= mapSize || otherY < 0 || otherY >= mapSize) {
                    continue;
                }

                SuperRoom nextSuperRoom = superRooms[otherY][otherX];
                if (!currentSuperRoom.getState().hasDoor(direction) || visited.contains(nextSuperRoom)) {
                    continue;
                }
        ...
        if (visited.size() < minRoomCount) {
            throw new GenerationFailedException("Not enough connected quantum rooms : " + visited.size());
        } else {
            log.info("Map generated with {} rooms", visited.size());
        }
        ...
        for (SuperRoom qRoom : visited) { // set all the connections
            ...
            for (int direction = 0; direction < 4; direction++) {
                if (!qRoom.getState().hasDoor(direction)) {
                    continue;
                }
