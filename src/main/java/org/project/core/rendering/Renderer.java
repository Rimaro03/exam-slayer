...
public class Renderer implements ComponentListener {
     ...
     */
    public static void addCircleToRenderQueue(Vec2 position, float radius, Color color, int priority) {
        getInstance().renderQueue.add(new RenderableCircle(radius, color, position, priority));
    }
     ...
     * @param priority The priority of the circle higher priority will be rendered last
     */
    public static void addImageToRenderQueue(Vec2 position, BufferedImage sprite, int priority) {
        getInstance().renderQueue.add(new RenderableImage(sprite, position, priority));
    }
     ...
     * @param priority The priority of the circle higher priority will be rendered last
     */
    public static void addTextToRenderQueue(Vec2 position, String text, Color color, int size, int priority) {
        getInstance().renderQueue.add(new RenderableText(text, position, size, color, priority));
    }
    ...
    public static void present(Graphics g) {
        getInstance().presentInternal(g);
    }

    /**
     * Converts a world x position to a screen x position
     */
    public static int worldToScreenX(float x) {
        return getInstance().worldToScreenXInternal(x);
    }

    /**
     * Converts a world y position to a screen y position
     */
    public static int worldToScreenY(float y) {
        return getInstance().worldToScreenYInternal(y);
    }

    /**
     * Converts a world size to a screen size
     */
    public static int worldToScreenSize(float size) {
        return getInstance().worldToScreenSizeInternal(size);
    }
